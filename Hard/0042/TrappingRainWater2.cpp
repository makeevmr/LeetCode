...
// of each bar is 1, compute how much water it can trap after raining. Time
// complexity: O(n * log(n)) Additional memory complexity: O(n) Idea: sort
// heights by non-decreasing and use prefix sums
...
public:
  static int trap(const std::vector<int>& height) {
    ...
    std::vector<std::pair<int, int>> sorted_heights(height_size,
    ...
    for (std::size_t i = 1; i < height_size; ++i) {
      ...
      if (sorted_heights[i].second < left_border) {
        max_water_capacity = (left_border - sorted_heights[i].second - 1) *
                             sorted_heights[i].first;
        total_trapping_water += max_water_capacity -
                                height_prefix_sums[left_border - 1] +
                                height_prefix_sums[sorted_heights[i].second];
        left_border = sorted_heights[i].second;
      }
      if (sorted_heights[i].second > right_border) {
        max_water_capacity = (sorted_heights[i].second - right_border - 1) *
                             sorted_heights[i].first;
        total_trapping_water +=
            max_water_capacity -
