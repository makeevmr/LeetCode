...
line. Time complexity: O(n ^ 2) Additional memory complexity: O(n) Idea: fix one
point, compute all inclines of all line with this fixed point and store them in
hashmap
*/
...
class Solution {
public:
  [[nodiscard]] static int maxPoints(
    ...
    for (int i = 0; i < points_size; ++i) {
      ...
      for (int j = i + 1; j < points_size; ++j) {
        ...
        answer =
            std::max(answer, inclines_map[incline_numer][incline_denom] + 1);
      }
    }
    return answer;
  }
