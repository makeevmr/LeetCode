/* Given an array of integers nums and an integer k, return the number of
 subarrays of nums where the bitwise AND of the elements of the subarray equals
 k. Time complexity: O(n * 32) = O(n), where n is size of nums array Additional
 memory complexity: O(1) Idea: use two pointers to store current subarray, use
 array of 32 elements to store number of occurances of each bit in subarray,
 count subarrays with number >= k and count subarrays with number >= k + 1
 */
...
private:
  ...
  [[nodiscard]] static int setNumber(
      const std::array<u_int32_t, bits_amount>& bits_counter) {
    int result = 0;
  ...
  static void addToBitsCounter(std::array<u_int32_t, bits_amount>& bits_counter,
                               int number) {
    for (int i = 0; i < bits_amount; ++i) {
      if (!(number & (1 << i))) {
  ...
  static void removeFromBitsCounter(
      std::array<u_int32_t, bits_amount>& bits_counter, int number) {
    for (int i = 0; i < bits_amount; ++i) {
      if (!(number & (1 << i))) {
        --bits_counter[bits_amount - i - 1];
      }
