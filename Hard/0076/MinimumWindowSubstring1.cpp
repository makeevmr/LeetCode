...
// Idea: use two pointers (left, right), move right pointer until meet a valid
// substr, then move left pointer until substr becomes invalid once again
...
public:
  static std::string minWindow(const std::string& s, const std::string& t) {
    ...
    return min_window.first == -1
               ? ""
               : s.substr(min_window.first, min_window.second);
  }
};
