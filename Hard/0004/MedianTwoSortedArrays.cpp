...
Idea: use binary search by answer as outer loop in nums1 than find that number
in nums2 in inner loop
*/
...
public:
  [[nodiscard]] double findMedianSortedArrays(const std::vector<int>& nums1,
    ...
    if ((nums1_size + nums2_size) % 2 != 0) {
      int answer =
          findKStatistic(nums1, nums2, (nums1_size + nums2_size - 1) / 2);
      if (answer == -1) {
        answer =
            findKStatistic(nums2, nums1, (nums1_size + nums2_size - 1) / 2);
      }
      ...
      return (static_cast<double>(answer1) + static_cast<double>(answer2)) /
             2.0;
    }
  }
...
private:
  [[nodiscard]] int static findKStatistic(const std::vector<int>& nums1,
    ...
    while (left1 < right1) {
      ...
      int max_statistic = right2 < static_cast<int>(nums2.size()) &&
                                  nums2.at(right2) == nums1.at(middle1)
                              ? middle1 + right2 + 1
                              : middle1 + right2;
    ...
    int max_statistic = right2 < static_cast<int>(nums2.size()) &&
                                nums2.at(right2) == nums1.at(left1)
                            ? left1 + right2 + 1
                            : left1 + right2;
    if (min_statistic <= k && k <= max_statistic) {
      return nums1[left1];
  ...
  [[nodiscard]] static int leftBinarySearch(const std::vector<int>& nums,
                                            int target) {
    int left = 0;
  ...
  [[nodiscard]] static int rightBinarySearch(const std::vector<int>& nums,
                                             int target) {
    int left = 0;
