...
Capital, LeetCode would like to work on some projects to increase its capital before the IPO. Since
it has limited resources, it can only finish at most k distinct projects before the IPO. Help
...
Idea: sort array {capital, profits}, create max_heap and insert all available profits at the moment
*/
...
class Solution {
public:
    [[nodiscard]] static int findMaximizedCapital(int k, int w, const std::vector<int>& profits,
