...
// justified. Time complexity solution: O(n), where n is number of words
...
class Solution {
public:
  static std::vector<std::string> fullJustify(std::vector<std::string>& words,
                                              int max_width) {
...
private:
  static std::size_t getSymbolsAmount(const std::vector<std::string>& words,
                                      int& curr_word, std::size_t u_max_width) {
    int words_amount = static_cast<int>(words.size());
  ...
  static void pushJustifiedLine(std::vector<std::string>& words,
                                std::vector<std::string>& justified_text,
                                std::string& justified_line,
                                std::size_t u_max_width, int start_line_word,
                                int curr_word, int space_length,
...
int main() {
  ...
  //                                   "justification."};
  // int max_width = 16;
  // std::vector<std::string> words = {"What", "must", "be", "acknowledgment",
  // "shall", "be"}; int max_width = 16;
  ...
  int max_width = 20;
  std::vector<std::string> justified_text =
      Solution::fullJustify(words, max_width);
