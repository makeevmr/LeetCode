...
return it. Time complexity: (k * log(k) * lists[i].length) because merge sort of
k elements is k * log(k), and leaf nodes lists have lists[i].length size
Additional memory complexity: O(log(k))
Idea: Divide & Conquer approach. Merge sort idea. Split array in two parts until
only 2 lists remains, merge all lists when climbing the recursion tree
*/
