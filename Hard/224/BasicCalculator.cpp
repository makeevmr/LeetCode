...
private:
    ...
    static const std::unordered_map<char, std::function<int(const int &, const int &)>> fn_map;

    static void calcValue(std::stack<Operator> &operations, std::stack<int> &values) {
    ...
    static void pushValue(std::stack<Operator> &operations, std::stack<int> &values,
                          int &curr_value, bool &is_curr_value_empty) {
        if (operations.empty() || operations.top().oper_type == '(') {
            ...
            if (operations.top() == Operator{'-', false}) {
                ...
                values.top() = fn_map.at(operations.top().oper_type)(values.top(), curr_value);
            }
            operations.pop();
        }
    ...
    friend bool operator==(const Solution::Operator &left, const Solution::Operator &right);
};

const std::unordered_map<char, std::function<int(const int &, const int &)>> Solution::fn_map = {
    {'+', [](const int &left, const int &right) { return left + right; }},
    {'-', [](const int &left, const int &right) { return left - right; }}};

bool operator==(const Solution::Operator &left, const Solution::Operator &right) {
    return ((left.oper_type == right.oper_type) && (left.is_binary == right.is_binary));
}
