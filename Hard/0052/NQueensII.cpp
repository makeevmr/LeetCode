...
puzzle. Time complexity: O(n!) Additional memory complexity: O(n) Idea: use
permutation of numbers 0...(n - 1) to represent queens placement (index of
number is column, number itself is row), use array of main_diad, secondary_diag
to represent engaged diagonals
*/
...
public:
  [[nodiscard]] static int totalNQueens(int n) {
    ...
    getNumberArrangements(curr_permutation, main_diag, secondary_diag, n,
                          arrangemet_number, 0);
...
private:
  static void getNumberArrangements(std::vector<int>& curr_permutation,
                                    ...
                                    std::vector<bool>& secondary_diag,
    ...
    for (int i = level; i < board_size; ++i) {
      if (!main_diag[curr_permutation[i] - level + board_size - 1] &&
        ...
        getNumberArrangements(curr_permutation, main_diag, secondary_diag,
                              board_size, arrangemet_number, level + 1);
        std::swap(curr_permutation[level], curr_permutation[i]);
