      ...
      for (int column = 0; column < board_columns; ++column) {
        if (trie.root->descendant[board[row][column] - 'a']) {
          ...
          DFS(board, words_in_board, curr_word,
              trie.root->descendant[board[row][column] - 'a'], board_rows,
              board_columns, row, column);
        }
      }
    }
...
private:
  ...
  static void DFS(std::vector<std::vector<char>>& board,
                  std::vector<std::string>& words_in_board,
                  std::string& curr_word, Trie<ALPHABET_SIZE>::Node* curr_node,
                  const int& board_rows, const int& board_columns, int row,
                  int column) {
...
const std::pair<int, int> offset_array[4] = {
    std::pair<int, int>{-1, 0}, std::pair<int, int>{0, 1},
    std::pair<int, int>{1, 0}, std::pair<int, int>{0, -1}};

const std::vector<std::pair<int, int>> Solution::offset(offset_array,
                                                        offset_array + 4);
