...
public:
  [[nodiscard]] static int maximalPathQuality(
    ...
    for (const auto& edge : edges) {
      graph[edge[0]].push_back(Destination{.node_ = edge[1], .time_ = edge[2]});
      graph[edge[1]].push_back(Destination{.node_ = edge[0], .time_ = edge[2]});
    }
    std::vector<int> node_visits(total_nodes, 0);
    node_visits[0] = 1;
    return getMaxQuality(values, graph, node_visits, 0, values[0], 0, max_time);
  }
  ...
  [[nodiscard]] static int getMaxQuality(
    ...
    for (const Destination& dest : graph[start_node]) {
      if (int new_time = curr_time + dest.time_; new_time <= max_time) {
        ...
        max_quality = std::max(
            max_quality, getMaxQuality(values, graph, node_visits, dest.node_,
