...
such that: Every adjacent pair of words differs by a single letter. Every si for
1 <= i <= k is in wordList. Note that beginWord does not need to be in wordList.
sk == endWord
Given two words, beginWord and endWord, and a dictionary wordList, return the
number of words in the shortest transformation sequence from beginWord to
endWord, or 0 if no such sequence exists. Time complexity: O(wordList_size ^ 2 *
beginWord_size) Additional memory complexity: O(wordList_size * beginWord_size)
Idea: use BFS to find shortest path in unweighted graph between beginWord and
endWord
*/
...
private:
  static bool isAdjacentPair(const std::string& word_l,
                             const std::string& word_r) {
    int diff = 0;
