...
one single character changed in the gene string. For example, "AACCGGTT" -->
"AACCGGTA" is one mutation. There is also a gene bank bank that records all the
valid gene mutations. A gene must be in bank to make it a valid gene string.
...
there is no such a mutation, return -1. Time complexity: O((bank_size ^ 2) *
gen_size) Additional memory complexity: O(bank_size * gen_size) Idea: use BFS to
find min_path in graph
*/
...
class Solution {
public:
  static int minMutation(const std::string& startGene,
    ...
    while (!queue.empty()) {
      ...
      for (std::size_t i = 0; i < queue_size; ++i) {
        ...
        for (const auto& new_gen : bank) {
          if (!visited_nodes.contains(new_gen) &&
              isMutateable(curr_gen, new_gen)) {
            queue.push(new_gen);
          }
        }
...
private:
  static bool isMutateable(const std::string& from_gen,
                           const std::string& to_gen) {
    int diff = 0;
