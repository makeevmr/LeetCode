...
Time complexity: O(t * 2 ^ t)
Additional memory complexity: O(t)
Idea: on each step we either add number to curr_comb from candidates[curr_ind] or move to next
...
private:
    void getSumCombinations(const std::vector<int>& candidates, std::vector<int>& curr_comb,
                            const int& target, int curr_sum, int curr_ind) {
        if (curr_sum == target) {
        ...
        getSumCombinations(candidates, curr_comb, target, curr_sum + candidates[curr_ind],
                           curr_ind);
        curr_comb.pop_back();
