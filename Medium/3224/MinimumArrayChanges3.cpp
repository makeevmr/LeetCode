...
k. You can perform some changes on the array, where in one change you can
replace any element in the array with any integer in the range from 0 to k. You
need to perform some changes (possibly none) such that the final array satisfies
the following condition: There exists an integer X such that abs(a[i] - a[n - i
- 1]) = X for all (0 <= i < n). Return the minimum number of changes required to
...
public:
  static int minChanges(const vector<int>& nums, int k) {
    ...
    for (const auto& pair : sorted_freq) {
      ...
      int curr_answer = 0;
      while (curr_ind < nums_size / 2) {
        if (abs(nums[curr_ind] - nums[nums_size - curr_ind - 1]) !=
            pair.second) {
