/* You are given an integer array nums of size n where n is even, and an integer
k.

You can perform some changes on the array, where in one change you can replace
any element in the array with any integer in the range from 0 to k.

You need to perform some changes (possibly none) such that the final array
satisfies the following condition:

There exists an integer X such that abs(a[i] - a[n - i - 1]) = X for all (0 <= i
< n). Return the minimum number of changes required to satisfy the above
condition. */
  ...
  static int minChanges(const vector<int>& nums, int k) {
    ...
    for (auto& item : max_diff_map) {
      if (pref_sums_max_diff.empty()) {
        ...
        pref_sums_max_diff.push_back(pair<int, int>{
            item.first, item.second + pref_sums_max_diff.back().second});
      }
    }
    ...
    for (const auto& item : sorted_abs_freq) {
      ...
      auto f_iter =
          lower_bound(pref_sums_max_diff.begin(), pref_sums_max_diff.end(),
                      item.second, [](const pair<int, int>& left, int right) {
      ...
      ull new_min_answer = less_max_diff * 2 +
                           static_cast<ll>(nums_size / 2 - less_max_diff) -
                           item.first;
