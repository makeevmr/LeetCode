...
public:
  static int snakesAndLadders(std::vector<std::vector<int>>& board) {
    ...
    std::vector<std::vector<std::pair<bool, bool>>> visited_squares(
        board_size,
        std::vector<std::pair<bool, bool>>(board_size, {false, false}));
  ...
  static int BFS(
    ...
    while (!queue.empty()) {
      ...
      for (std::size_t i = 0; i < queue_size; ++i) {
        ...
        queue.pop();
        for (int new_ind = curr_ind + 1,
                 max_turn_ind = std::min(curr_ind + 6, max_ind);
          ...
          if (!visited_squares[dest1_coord.first][dest1_coord.second].first) {
            ...
            if ((!dest_with_transition &&
                 !visited_squares[dest2_coord.first][dest2_coord.second]
                      .first &&
                 !visited_squares[dest2_coord.first][dest2_coord.second]
                      .second) ||
                (dest_with_transition &&
                 ((single_transition &&
                   !visited_squares[dest1_coord.first][dest1_coord.second]
                        .first) ||
                  (!single_transition &&
                   !visited_squares[dest2_coord.first][dest2_coord.second]
                        .second)))) {
