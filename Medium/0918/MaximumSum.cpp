...
next element of nums[i] is nums[(i + 1) % n] and the previous element of nums[i] is nums[(i - 1 + n)
% n].
...
Idea: if max_subarray inside nums than answer is MaxKadane(nums) algorithm, if max_sum in nums
edges than answer is total_sum - MinKadane(nums)
*/
...
class Solution {
public:
    [[nodiscard]] static int maxSubarraySumCircular(const std::vector<int>& nums) {
...
private:
    ...
    [[nodiscard]] static int getTotalSum(const std::vector<int>& nums, bool& is_all_neg) {
        int total_sum = 0;
