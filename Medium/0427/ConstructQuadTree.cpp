...
node on the tree or False if the node has four children. Time complexity: O(n ^
2 * log(n)) Additional memory complexity: O(log(n)) Idea: if square is not leaf
node create 4 descendants, and check them recursively
*/
...
public:
  ...
  Node(bool val, bool is_leaf, Node* top_left, Node* top_right,
...
private:
  ...
  static Node* constructTree(const std::vector<std::vector<int>>& grid,
    ...
    if (!root->isLeaf) {
      ...
      root->topLeft =
          constructTree(grid, top_left, std::pair<int, int>{x_cent, y_cent});
      root->topRight =
          constructTree(grid, std::pair<int, int>{x_cent, top_left.second},
                        std::pair<int, int>{bot_right.first, y_cent});
      root->bottomLeft =
          constructTree(grid, std::pair<int, int>{top_left.first, y_cent},
                        std::pair<int, int>{x_cent, bot_right.second});
      root->bottomRight =
          constructTree(grid, std::pair<int, int>{x_cent, y_cent}, bot_right);
    } else {
