...
and column to 0's. You must do it in place. Time complexity: O(rows * columns)
Additional memory complexity: O(1)
Idea: use first row and first column to store zero flags if corresponding row or
column must be filled with zeros */
...
private:
  ...
  static void fillColumn(std::vector<std::vector<int>>& matrix,
                         std::size_t column) {
