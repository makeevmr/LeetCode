...
Additional memory complexity: O(V + E)
Idea: use DFS algorithm to find cycles in graph and topological sort
*/
...
class Solution {
public:
    static std::vector<int> findOrder(int numCourses,
        ...
        for (int i = 0; i < numCourses; ++i) {
            if (visited_nodes[i] == 0) {
                if (topologicalSortDFS(adjacency_list, visited_nodes, topological_sort, i)) {
                    cycle_found = true;
                    break;
                }
            }
...
private:
    ...
    static bool
    topologicalSortDFS(const std::unordered_map<int, std::unordered_set<int>> &adjacency_list,
                       std::vector<int> &visited_nodes, std::vector<int> &topological_sort,
                       int node) {
        visited_nodes[node] = 1;
        if (auto f_iter = adjacency_list.find(node); f_iter != adjacency_list.end()) {
