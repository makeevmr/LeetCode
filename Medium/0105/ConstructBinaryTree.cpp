...
public:
  static TreeNode* buildTree(const std::vector<int>& preorder,
    ...
    int curr_ind = 0;
    return buildTreeFromNode(preorder, inorder, preorder_ind_m, preorder_size,
                             curr_ind);
  }
...
private:
  static TreeNode* buildTreeFromNode(
    ...
    while (curr_ind < inorder_size) {
      if (preorder_ind_m.at(curr_node->val) <
          preorder_ind_m.at(inorder[curr_ind])) {
        curr_node->right = buildTreeFromNode(preorder, inorder, preorder_ind_m,
