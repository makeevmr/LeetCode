...
burst all balloons. Time complexity: O(n * log(n)) Additional memory complexity:
O(log(n)) since std::sort use QuickSort implementation Idea: sort intervals by
left[0] < right[0], if equals then left[1] > right[1], iterate through points
