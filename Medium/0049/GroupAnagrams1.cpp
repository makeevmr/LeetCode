...
letters exactly once. Time complexity: O(n * k), where n is size of strs, k is
size of word Additional memory complexity: O(n * k) Idea: iterate through
vector, sort each word using counting sort, push new word to the unordered_map
...
public:
  static std::vector<std::vector<std::string>> groupAnagrams(
    ...
    for (auto b_iter = anagrams_map.begin(); b_iter != anagrams_map.end();
         ++b_iter) {
