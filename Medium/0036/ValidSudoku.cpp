...
private:
  ...
  static bool isValidRow(const std::vector<std::vector<char>>& board,
                         const int row) {
  ...
  static bool isValidColumn(const std::vector<std::vector<char>>& board,
                            const int column) {
    std::vector<bool> is_contains_number(10, false);
    for (int row = 0; row < sudoku_size; ++row) {
      if (board[row][column] == '.') {
  ...
  static bool isValidBox(const std::vector<std::vector<char>>& board,
                         const int box_number) {
