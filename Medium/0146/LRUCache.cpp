...
functions get and put must each run in O(1) average time complexity. Time
complexity: get is O(1) on average, put is O(1) on averave Additional memory
complexity: O(capacity) Idea: use unordered_map<int, Node *> to access key in
O(1), Node is node of doubly linked list to change last use of node in O(1)
*/
...
private:
  ...
  public:
    ...
    void updateValue(Node* const node_to_update,
