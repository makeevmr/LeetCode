...
Idea: let's consider current min pairs with fixed nums1_ind. It's pairs indexes
are: (0, 0), (1, 0), (2, 0) ... (nums1_size - 1, 0) We contain sums of these
pairs in min_heap, pop min_pair (i, j) and push sum that correspond to pair(i, j
+ 1) into heap
*/
...
class Solution {
public:
  [[nodiscard]] static std::vector<std::vector<int>> kSmallestPairs(
    ...
    while (nums1_ind < nums1_size && nums1_ind < k) {
      pairs_sum_min_heap.push(
          std::pair<int, int>{nums1[nums1_ind] + nums2[0], nums1_ind});
      ++nums1_ind;
    }
    while (k > 0) {
      const std::pair<int, int>& top_pair = pairs_sum_min_heap.top();
      answer.push_back(
          {nums1[top_pair.second], nums2[curr_pairs[top_pair.second]]});
