/* According to Wikipedia's article: "The Game of Life, also known simply as Life, is a cellular
   automaton devised by the British mathematician John Horton Conway in 1970. The next state is
   created by applying the above rules simultaneously to every cell in the current state, where
   births and deaths occur simultaneously. Given the current state of the m x n grid board,
...
private:
    static bool getNewState(const std::vector<std::vector<int>> &board, const int &row,
                            const int &column, const int &rows, const int &columns) {
