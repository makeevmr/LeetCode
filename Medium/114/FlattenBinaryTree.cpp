...
next node in the list and the left child pointer is always null. The "linked list" should be in the
same order as a pre-order traversal of the binary tree.
Time complexity: O(n)
Additional memory complexity: O(height of BT)
Idea: return pair(head, tail) on each straightenTree() call
*/
