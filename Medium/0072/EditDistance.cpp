...
Idea: use dynamic programming approach (dp_matrix[i][j] contain min edit for
word1[0..i), word2[0..j))
*/
...
class Solution {
public:
  [[nodiscard]] static int minDistance(const std::string& word1,
    ...
    for (int i = 1; i <= word1_size; ++i) {
      ...
      for (int j = 1; j <= word2_size; ++j) {
        ...
        if (word1[i - 1] == word2[j - 1]) {
          ...
          dp_array[j] =
              std::min(std::min(dp_array[j], dp_array[j - 1]), prev_diag_val) +
              1;
        }
