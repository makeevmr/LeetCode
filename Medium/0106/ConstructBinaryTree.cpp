...
public:
    static TreeNode *buildTree(std::vector<int> &inorder, std::vector<int> &postorder) {
        ...
        return buildTreeFromNode(inorder, postorder, postorder_ind_m, postorder_size, curr_ind);
    }

private:
    static TreeNode *buildTreeFromNode(const std::vector<int> &inorder,
        ...
        while (curr_ind >= 0) {
            if (postorder_ind_m.at(curr_node->val) > postorder_ind_m.at(inorder[curr_ind])) {
                curr_node->left =
                    buildTreeFromNode(inorder, postorder, postorder_ind_m, inorder_size, curr_ind);
            } else if (int postorder_ind = postorder_ind_m.at(inorder[curr_ind]);
                       (postorder_ind > 0) && postorder[postorder_ind - 1] == curr_node->val) {
