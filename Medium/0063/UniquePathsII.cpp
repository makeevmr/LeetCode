...
Idea: use dynamic programming approach (unique_paths[column] store unique_paths from (0, 0) to
(curr_row, column) on each row)
*/
...
public:
    [[nodiscard]] static int uniquePathsWithObstacles(
        ...
        for (int column = 1; column < columns; ++column) {
            unique_paths.push_back((obstacle_grid[0][column] == 1) ? 0 : unique_paths[column - 1]);
        }
