...
Time complexity: (s1_size * s2_size)
Additional memory complexity: O(s2_size)
Idea: use dynamic programming approach (dp_matrix[i][j] equals true if we can construct s3[0...(i +
j + 1)] with i + 1 first symbols from s1 and j + 1 first symbols from s2) use dp_array instead
dp_matrix because no need to use O(s1_size * s2_size) additional memory
*/
...
class Solution {
public:
    [[nodiscard]] static bool isInterleave(const std::string& s1, const std::string& s2,
                                           const std::string& s3) {
