...
directories. Return the new path. Time complexity: O(n), where n is length of
path Additional memory complexiy: O(n) Idea: use stack to store path in right
order
*/
...
private:
  static void changePath(std::stack<std::string>& simplified_path,
                         std::string& curr_direcory) {
