...
may not be used more than once. Time complexity: O(rows * columns * 3 ^
(word_size - 1)) Additional memory complexity: O(word_size) Idea: use DFS to
consider all possible ways in graph, store met cells in board
*/
...
private:
  [[nodiscard]] static bool DFS(std::vector<std::vector<char>>& board,
                                const std::string& word, const int& rows,
                                const int& columns, const int& word_size,
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if ((0 <= new_row && new_row < rows) &&
          (0 <= new_column && new_column < columns) &&
          board[new_row][new_column] != '#' &&
          DFS(board, word, rows, columns, word_size, new_row, new_column,
              word_ind + 1)) {
        return true;
      }
    }
  ...
  static constexpr const std::pair<int, int> offsets[4] = {
      {-1, 0}, {0, 1}, {1, 0}, {0, -1}};
};
