...
time. Time complexity: O(n) Additional memory complexity: O(n) Idea: use 2
unordered_maps. First contains start number of sequence as key and length of
sequence as value Second contains end number of sequence as key and length of
sequence as value Iterate through nums and fill unordered_maps, then while
unordered_maps is not empty concatinate sequences
*/
  ...
  static int longestConsecutive(const std::vector<int>& nums) {
    ...
    while (!seq_start.empty()) {
      ...
      while (seq_start.contains(b_iter->first + b_iter->second)) {
        ...
        seq_end[f_iter_start->first + f_iter_start->second - 1] +=
            b_iter->second;
