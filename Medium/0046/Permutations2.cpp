...
+ 1)) Additional memory complexity: O(n) Idea: use available_numbers array to
capture leftmost available number
*/
...
public:
  static std::vector<std::vector<int>> permute(const std::vector<int>& nums) {
    ...
    getPermutations(permutations, curr_perm, nums, available_numbers, nums_size,
                    0);
    return permutations;
  }
...
private:
  static void getPermutations(std::vector<std::vector<int>>& permutations,
                              ...
                              std::vector<bool>& available_numbers,
                              const int& nums_size, int level) {
    if (level == nums_size) {
    ...
    for (int i = 0; i < nums_size; ++i) {
      ...
      getPermutations(permutations, curr_perm, nums, available_numbers,
                      nums_size, level + 1);
