...
Idea: compute min_paths on each step (min_paths[i] contains min sum on path from
last_row to current_row ended with triangle[current_row][i])
*/
...
public:
  [[nodiscard]] static int minimumTotal(
    ...
    // min_paths[i] contains min sum on path from last_row to current_row ended
    // with
    std::vector<int> min_paths = triangle.back();
    for (int row = triangle_width - 2; row >= 0; --row) {
      for (int column = 0; column <= row; ++column) {
        min_paths[column] = std::min(min_paths[column], min_paths[column + 1]) +
                            triangle[row][column];
      }
    }
    return min_paths[0];
