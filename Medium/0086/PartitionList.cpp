...
before nodes greater than or equal to x. You should preserve the original relative order of the
nodes in each of the two partitions.
Time complexity: O(n)
Additional memory complexity: O(1)
Idea: create two lists less_x and not_less_x, add new node to one of them depending on the value x
*/
...
private:
    static void addNode(ListNode *&head, ListNode *&curr_node, ListNode *const &node_to_add) {
        if (head == nullptr) {
            head = node_to_add;
        } else {
