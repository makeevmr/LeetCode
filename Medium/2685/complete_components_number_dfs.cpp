...
public:
  [[nodiscard]] static int countCompleteComponents(
    ...
    for (int i = 0; i < n; ++i) {
      if (component[i] == 0) {
        ++total_components;
        if (isCompleteComponent(adj_list, component, total_components, i)) {
          ++complete_components;
        }
      }
    }
...
private:
  ...
  [[nodiscard]] static std::size_t dfs(
    ...
    ++vertex_num_in_comp;
    if (const auto &f_iter = adj_list.find(vertex_ind), e_iter = adj_list.end();
        f_iter != e_iter) {
  ...
  [[nodiscard]] static bool isCompleteComponent(
    ...
    std::size_t edges = dfs(adj_list, component, total_components, vertex_ind,
                            is_equal_edges_num, vertex_num_in_comp);
