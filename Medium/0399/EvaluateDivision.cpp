...
Time complexity: O(Q * (E + V)), where Q is number of queries, E - edges, V - vertices in graph
Memory complexity: O(V^2)
Idea: use biderectional graph to represent all equations, BFS to find value of equation
*/
...
class Solution {
public:
    static std::vector<double> calcEquation(std::vector<std::vector<std::string>> &equations,
                                            std::vector<double> &values,
                                            std::vector<std::vector<std::string>> &queries) {
        std::unordered_map<std::string, std::unordered_map<std::string, double>> adjacency_list;
        ...
        for (std::size_t i = 0; i < queries.size(); ++i) {
            if (adjacency_list.contains(queries[i][0]) && adjacency_list.contains(queries[i][1])) {
...
private:
    static void BFS(const std::unordered_map<std::string, std::unordered_map<std::string, double>>
                        &adjacency_list,
        ...
        while (!is_answer_set && !unvisited_nodes.empty()) {
            std::pair<std::string, double> new_node = std::move(unvisited_nodes.front());
            unvisited_nodes.pop();
...
int main() {
    ...
    std::vector<std::vector<std::string>> queries = {
        {"a", "c"}, {"b", "d"}, {"b", "a"}, {"d", "c"}};
    std::vector<double> query_answers = Solution::calcEquation(equations, values, queries);
