...
below. Note that 1 does not map to any letters. Time complexity: O(4 ^
(digits_size) * digits_size) Additional memory complexity: O(digits_size) Idea:
use recursion to find all possible combinations of letters
*/
...
class Solution {
public:
  static std::vector<std::string> letterCombinations(
...
private:
  static void getAllCombinations(const std::string& digits,
    ...
    for (const auto& letter : letters_map.find(digits[level] - '0')->second) {
      ...
      getAllCombinations(digits, combinations_array, curr_combination,
                         level + 1);
    }
  }
