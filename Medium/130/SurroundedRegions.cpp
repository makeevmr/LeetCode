...
none of the region cells are on the edge of the board. A surrounded region is captured by replacing
all 'O's with 'X's in the input matrix board.
Time complexity: O(rows * columns)
Memory complexity: O(rows * columns)
Idea: use DFS algorithm
*/
...
private:
    static void isSurroundedRegion(std::vector<std::vector<char>> &board, int row, int column,
                                   const int &rows, const int &columns, bool &is_surrounded) {
    ...
    static void paintOver(std::vector<std::vector<char>> &board, int row, int column,
                          const int &rows, const int &columns, const char &color) {
        if (row < 0 || column < 0 || row == rows || column == columns ||
