...
public:
  ...
  void addWord(const std::string& word) {
    ...
    if (word_ind == word.size()) {
      ...
      node_ptr->node_map.insert(
          {word[word_ind], new Node(true, word.substr(word_ind))});
    }
  }
...
private:
  [[nodiscard]] bool isSuffixFound(const std::string& word,
                                   std::size_t word_ind,
                                   const std::size_t& word_size,
                                   Node* curr_node) const noexcept {
  ...
  [[nodiscard]] EndNodeInfo findEndNode(
    ...
    while (pref_ind < prefix_size) {
      ...
      if (data_ind < data_size || pref_ind == prefix_size ||
        ...
        return {
            .data_ind = data_ind, .pref_ind = pref_ind, .node_ptr = curr_node};
      }
  ...
  public:
    ...
    Node(bool terminal, const std::string& data)
        : terminal(terminal),
          data(data),
          node_map() {}
