...
pointers in the new list should point to nodes in the original list. Time
complexity: O(n) Additional memory complexity: O(n) Idea: use unordered_map<Node
*, int> to store relative indexes of nodes, use vector to store relative
...
public:
  static Node* copyRandomList(const Node* head) {
    ...
    curr_index = 0;
    while (curr_node_orig != nullptr) {
      random_ptrs[curr_index] =
          relative_ind_orig.contains(curr_node_orig->random)
