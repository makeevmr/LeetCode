...
Idea: use std::pair to determine the boundary of possible values for each subtree
*/
...
private:
    [[nodiscard]] static bool isValidSubtree(TreeNode *root,
                                             std::pair<long long int, long long int> border) {
        if (root == nullptr) {
            return true;
        }
