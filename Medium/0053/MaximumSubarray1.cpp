...
count prefix sum on each step, subarray_sum = pref1_sum - pref2_sum, so we
compute curr_pref_sum on each step and keep min_pref_sum from[0...i]
*/
...
public:
  [[nodiscard]] static int maxSubArray(std::vector<int>& nums) {
    ...
    for (int i = 0; i < nums_size; ++i) {
      ...
      max_subarray_sum =
          std::max(max_subarray_sum, curr_pref_sum - min_pref_sum);
