...
Idea: use dynamic programming approach (grid_sum[i][j] stores min path sum from
...
class Solution {
public:
  [[nodiscard]] static int minPathSum(
    ...
    for (int i = 1; i < rows; ++i) {
      for (int j = 1; j < columns; ++j) {
        grid_sum[i][j] =
            grid[i][j] + std::min(grid_sum[i][j - 1], grid_sum[i - 1][j]);
      }
    }
