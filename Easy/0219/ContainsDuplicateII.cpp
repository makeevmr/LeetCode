...
j) <= k. Time complexity: O(n) Additional memory complexity: O(n) Idea: use
unordered_map to store last occurrences of each element (number, index)
*/
...
public:
  static bool containsNearbyDuplicate(const std::vector<int>& nums, int k) {
    ...
    for (std::size_t i = 0; i < nums.size(); ++i) {
      if (auto f_iter = last_occurrences.find(nums[i]);
          f_iter == last_occurrences.end()) {
