...
Idea: use unordered_map to store all possible values and their indexes, iterate through nums and, if
...
public:
    static std::vector<int> twoSum(const std::vector<int> &nums, int target) {
        ...
        for (std::size_t i = 1; i < nums_size; ++i) {
            ...
            if (auto f_iter = first_occurrences.find(nums[i]); f_iter == first_occurrences.end()) {
